...
namespace DYRMock.Models
{
    public class PetBuilder
    {
            ...
            Pets = new List<Pet>()
                {
                    new Pet()
                    {
                        Id = 1, Name = "Leo", LocationPoint = 35, IsFeatured = true, PetType = vm.PetTypes.FirstOrDefault(pt => pt.Id == 1), PetSize = PetSize.LARGE,
                        PetAge = PetAge.YOUNG, PetGender = PetGender.FEMALE,
                        Breed = new Breed() { Id = 1, BreedType = "American Foxhound" },
                        Facility = new Facility() { Id = 1, FacilityType = FacilityType.SHELTER, LocationCity = "Manhattan", LocationState = "NY" },
                    ...
                    new Pet()
                    {
                        Id = 2, Name = "Cali", LocationPoint = 6, IsFeatured = true, PetType = vm.PetTypes.FirstOrDefault(pt => pt.Id == 1), PetSize = PetSize.SMALL,
                        PetAge = PetAge.YOUNG, PetGender = PetGender.MALE,
                        Breed = new Breed() { Id = 2, BreedType = "West Highland Terrier" },
                        Facility = new Facility() { Id = 2, FacilityType = FacilityType.CERTIFIED_BREEDER, LocationCity = "Manhattan", LocationState = "NY" },
                    ...
                    new Pet()
                    {
                        Id = 3, Name = "Melika", LocationPoint = 12, IsFeatured = true, PetType = vm.PetTypes.FirstOrDefault(pt => pt.Id == 2), PetSize = PetSize.LARGE,
                        PetAge = PetAge.ADULT, PetGender = PetGender.FEMALE,
                        Breed = new Breed() { Id = 1, BreedType = "Domestic Short Hair" },
                        Facility = new Facility() { Id = 3, FacilityType = FacilityType.SHELTER, LocationCity = "Indiana", LocationState = "PA" },
        ...
        private void BuildFullLocation()
        {
            ...
            {
                if (!String.IsNullOrEmpty(p.Facility.LocationCity) && !String.IsNullOrEmpty(p.Facility.LocationState))
                {
                    p.Facility.FullLocation = p.Facility.LocationCity + ", " + p.Facility.LocationState;
                }
            }
        }
    }
